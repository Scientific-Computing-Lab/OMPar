...
inline uint64_t  __attribute__((always_inline)) getblock64(const uint8_t *p,uint32_t i)
{
  uint64_t s = 0;
  for (uint32_t n = 0; n <= ((unsigned int )8) - 1; n += 1) {
...
inline uint64_t  __attribute__((always_inline)) fmix64(uint64_t k)
{
  k ^= k >> 33;
...
void MurmurHash3_x64_128(const void *key,const uint32_t len,const uint32_t seed,void *out)
{
  ...
  for (uint32_t i = 0; i <= nblocks - 1; i += 1) {
...
// initialize the length array
  ...
  d_length[0] = 0;
  for (uint32_t i = 0; i <= numKeys - 1; i += 1) {
...
// sanity check
  for (uint32_t i = 0; i <= numKeys - 1; i += 1) {
    ((bool )(0 == memcmp((d_keys + d_length[i]),keys[i],length[i])))?((void )0) : __assert_fail("0 == memcmp(d_keys+d_length[i], keys[i], length[i])","murmurhash3.cpp",171,__PRETTY_FUNCTION__);
  }
{
    auto start = std::chrono::_V2::steady_clock::now();
    for (uint32_t n = 0; n <= repeat - 1; n += 1) {
      for (uint32_t i = 0; i <= numKeys - 1; i += 1) {
