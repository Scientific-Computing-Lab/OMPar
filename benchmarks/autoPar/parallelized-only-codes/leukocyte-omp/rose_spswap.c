...
#endif
{
  ...
  if (!A || !scan_row || !scan_idx || !col_list) 
    ev_err("spswap.c",8,59,"scan_to",0);
  if (scan_row -> dim != scan_idx -> dim || scan_idx -> dim != col_list -> dim) 
    ev_err("spswap.c",1,61,"scan_to",0);
  if (max_row < 0) 
    return ;
  if (!A -> flag_col) 
    sp_col_access(A);
  for (j_idx = 0; ((unsigned int )j_idx) <= scan_row -> dim - 1; j_idx += 1) {
    row_num = scan_row -> ive[j_idx];
    idx = scan_idx -> ive[j_idx];
    col = col_list -> ive[j_idx];
    if (col < 0 || col >= A -> n) 
      ev_err("spswap.c",2,76,"scan_to",0);
    if (row_num < 0) {
      idx = col;
      continue; 
    }
    r = &A -> row[row_num];
    if (idx < 0) 
      ev_err("spswap.c",17,84,"scan_to",0);
    e = &r -> elt[idx];
    if (e -> col != col) 
      ev_err("spswap.c",17,87,"scan_to",0);
    if (idx < 0) {
      printf("scan_to: row_num = %d, idx = %d, col = %d\n",row_num,idx,col);
      ev_err("spswap.c",17,92,"scan_to",0);
    }
...
#endif
{
  ...
  if (old_row >= 0) {
    r = &A -> row[old_row];
    old_idx = (old_idx >= 0 && old_idx < r -> len && r -> elt[old_idx] . col == col?old_idx : sprow_idx(r,col));
    e = &r -> elt[old_idx];
    e -> nxt_row = row_num;
    e -> nxt_idx = idx;
  }
...
#endif
{
  ...
  row_elt *e;
  if (col < 0 || col >= A -> n) 
    ev_err("spswap.c",2,157,"chase_col",0);
  tmp_row =  *row_num;
  if (tmp_row < 0) {
    if (A -> start_row[col] > max_row) {
      tmp_row = - 1;
      tmp_idx = col;
      return (row_elt *)((void *)0);
    }
     else {
...
#ifdef DEBUG
#endif
      ev_err("spswap.c",17,193,"chase_col",0);
    }
    e = &r -> elt[tmp_idx];
    old_row = tmp_row;
    old_idx = tmp_idx;
...
/* use A->start_row[..] etc. */
{
    ...
    r = &A -> row[tmp_row];
    if (tmp_idx < 0 || tmp_idx >= r -> len || r -> elt[tmp_idx] . col != col) 
      ev_err("spswap.c",17,255,"chase_past",0);
  ...
  if (tmp_row < 0) 
    ...
    if (tmp_idx < 0 || tmp_idx >= A -> row[tmp_row] . len || A -> row[tmp_row] . elt[tmp_idx] . col != col) 
      ev_err("spswap.c",17,268,"bump_col",0);
...
/* printf("bump_col: col = %d, row# = %d, idx = %d\n",
  ...
  if (tmp_row < 0) {
    ...
    r = &A -> row[tmp_row];
    if (tmp_idx < 0 || tmp_idx >= r -> len || r -> elt[tmp_idx] . col != col) 
      ev_err("spswap.c",17,303,"bump_col",0);
    e = &r -> elt[tmp_idx];
    tmp_row = e -> nxt_row;
    tmp_idx = e -> nxt_idx;
  }
  if (tmp_row < 0) {
    ...
    if (tmp_idx < 0 || tmp_idx >= A -> row[tmp_row] . len || A -> row[tmp_row] . elt[tmp_idx] . col != col) 
      ev_err("spswap.c",17,317,"bump_col",0);
    e = &A -> row[tmp_row] . elt[tmp_idx];
  }
   *row_num = tmp_row;
   *idx = tmp_idx;
