...
void SimpleRecursiveGaussianRGBAHost(const unsigned int *uiDataIn,unsigned int *uiDataOut,int iWidth,int iHeight,float a)
{
  ...
  for (int X = 0; X <= iWidth - 1; X += 1) {
// forward pass
    rgbaUintToFloat4(uiDataIn[X],yp);
// previous output
    for (int Y = 0; Y <= iHeight - 1; Y += 1) {
...
void RecursiveGaussianRGBAHost(const unsigned int *uiDataIn,unsigned int *uiDataOut,int iWidth,int iHeight,float a0,float a1,float a2,float a3,float b1,float b2,float coefp,float coefn)
{
...
// previous output by 2
#ifdef CLAMP_TO_EDGE
    rgbaUintToFloat4(uiDataIn[X],xp);
    
#pragma omp parallel for firstprivate (coefp)
    for (int i = 0; i <= 3; i += 1) {
      yb[i] = xp[i] * coefp;
      yp[i] = yb[i];
    }
...
#pragma omp parallel for firstprivate (coefn)
    for (int i = 0; i <= 3; i += 1) {
      xa[i] = xn[i];
      yn[i] = xn[i] * coefn;
      ya[i] = yn[i];
...
void HostRecursiveGaussianRGBA(unsigned int *uiInputImage,unsigned int *uiTempImage,unsigned int *uiOutputImage,int iWidth,int iHeight,GaussParms *pGP)
{
// run the gaussian in one dimension
    #if USE_SIMPLE_FILTER
    #else   
  RecursiveGaussianRGBAHost(uiInputImage,uiTempImage,iWidth,iHeight,pGP -> a0,pGP -> a1,pGP -> a2,pGP -> a3,pGP -> b1,pGP -> b2,pGP -> coefp,pGP -> coefn);
    #endif
// transpose results
...
// note width and height flipped due to transpose
    #if USE_SIMPLE_FILTER
    #else 
  RecursiveGaussianRGBAHost(uiOutputImage,uiTempImage,iHeight,iWidth,pGP -> a0,pGP -> a1,pGP -> a2,pGP -> a3,pGP -> b1,pGP -> b2,pGP -> coefp,pGP -> coefn);
    #endif
...
// pre-compute filter coefficients
  pGP -> nsigma = fSigma;
// note: fSigma is range-checked and clamped >= 0.1f upstream
  pGP -> alpha = 1.695f / pGP -> nsigma;
  pGP -> ema = std::exp(-pGP -> alpha);
  pGP -> ema2 = std::exp(- 2.0f * pGP -> alpha);
  pGP -> b1 = - 2.0f * pGP -> ema;
  pGP -> b2 = pGP -> ema2;
  pGP -> a0 = 0.0f;
  pGP -> a1 = 0.0f;
    ...
    case 1:
{
    ...
    case 2:
{
      const float ea = std::exp(-pGP -> alpha);
