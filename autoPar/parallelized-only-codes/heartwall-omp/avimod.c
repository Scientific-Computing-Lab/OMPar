...
#pragma omp parallel for private (temp,j) firstprivate (height,width,top,left,scale)
...
#pragma omp parallel for private (temp,j) firstprivate (height,width,top,left)
      for (j = 0; j <= width_new - 1; j += 1) {
// colums
