...
{
  real t = ((real )(rand())) / ((real )2147483647);
...
int main(int argc,char **argv)
{
  ...
  TOptionData optionData[1024];
  real callValueBS[1024];
  real callValueGPU[1024];
  real callValueCPU[1024];
  real sumDelta;
  real sumRef;
  real gpuTime;
  real errorVal;
  ...
  binomialOptionsGPU(callValueGPU,optionData,OPT_N,1000);
  auto end = std::chrono::_V2::system_clock::now();
  struct std::chrono::duration< real  , class std::ratio< 1 , 1L >  > elapsed_seconds = (end-start);
  ...
  for (i = 0; i <= OPT_N - 1; i += 1) {
    sumDelta += std::fabs(callValueBS[i] - callValueGPU[i]);
    sumRef += std::fabs(callValueBS[i]);
  }
  if (sumRef > 1E-5) {
  ...
  sumRef = 0;
  for (i = 0; i <= OPT_N - 1; i += 1) {
    sumDelta += std::fabs(callValueBS[i] - callValueCPU[i]);
    sumRef += std::fabs(callValueBS[i]);
  }
  if (sumRef > 1E-5) {
    printf("L1 norm: %E\n",(sumDelta / sumRef));
  ...
  sumRef = 0;
  for (i = 0; i <= OPT_N - 1; i += 1) {
    sumDelta += std::fabs(callValueGPU[i] - callValueCPU[i]);
