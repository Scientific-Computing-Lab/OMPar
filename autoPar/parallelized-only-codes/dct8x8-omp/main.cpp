...
#include "DCT8x8.h"
#include <omp.h> 
void DCT8x8(float *d_Dst,const float *d_Src,unsigned int stride,unsigned int imageH,unsigned int imageW,int dir);

void Verify(const float *h_OutputGPU,float *h_OutputCPU,const float *h_Input,const unsigned int stride,const unsigned int imageH,const unsigned int imageW,const int dir)
{
...
#pragma omp parallel for private (j) reduction (+:sum,delta) firstprivate (imageH,imageW)
  for (unsigned int i = 0; i <= imageH - 1; i += 1) {
    
#pragma omp parallel for reduction (+:sum,delta) firstprivate (stride)
    for (unsigned int j = 0; j <= imageW - 1; j += 1) {
...
int main(int argc,char **argv)
{
  ...
  for (unsigned int i = 0; i <= imageH - 1; i += 1) {
    for (unsigned int j = 0; j <= imageW - 1; j += 1) {
      h_Input[i * stride + j] = ((float )(rand())) / ((float )2147483647);
    }
  }
{
    ...
    for (int iter = 0; iter <= numIterations - 1; iter += 1) {
      DCT8x8(h_OutputGPU,h_Input,stride,imageH,imageW,dir);
    }
