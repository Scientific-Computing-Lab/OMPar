...
{
  node *c;
  if (tree_queue == ((void *)0)) {
    tree_queue = new_node;
    tree_queue -> next = ((void *)0);
  }
   else {
    c = tree_queue;
    while(c -> next != ((void *)0)){
...
#pragma omp parallel for private (i) reduction (+:num_keys)
...
// OpenCL kernel
        kernel_wrapper(records,records_elem,knodes,knodes_elem,knodes_elem,order,maxheight,count,currKnode,offset,keys,ans);
//records_mem,
//knodes_mem,
...
#pragma omp parallel for private (i)
        ...
        kernel2_wrapper(knodes,knodes_elem,knodes_elem,order,maxheight,count,currKnode,offset,lastKnode,offset_2,start,end,recstart,reclength);
// knodes_mem,
        pFile = fopen(output,"aw+");
        if (pFile == ((void *)0)) {
          printf("Fail to open %s !\n",output);
