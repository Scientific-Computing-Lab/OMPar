...
void zgivens(
//x,y,c,s)
complex x,complex y,double *c,complex *s)
{
  double inv_norm;
  double norm;
  complex tmp;
...
ZVEC *rot_zvec(ZVEC *x,int i,int k,double c,complex s,ZVEC *out)
{
  ...
  complex temp2;
  if (x == ((ZVEC *)((void *)0))) 
    ev_err("zgivens.c",8,90,"rot_zvec",0);
  if (i < 0 || i >= x -> dim || k < 0 || k >= x -> dim) 
    ev_err("zgivens.c",10,92,"rot_zvec",0);
  if (x != out) 
    out = _zv_copy(x,out,0);
...
ZMAT *zrot_rows(
//mat,i,k,c,s,out)
ZMAT *mat,int i,int k,double c,complex s,ZMAT *out)
{
  unsigned int j;
  complex temp1;
  complex temp2;
  if (mat == ((ZMAT *)((void *)0))) 
    ev_err("zgivens.c",8,126,"zrot_rows",0);
  if (i < 0 || i >= mat -> m || k < 0 || k >= mat -> m) 
    ev_err("zgivens.c",10,128,"zrot_rows",0);
  if (mat != out) 
    out = _zm_copy(mat,(zm_resize(out,(mat -> m),(mat -> n))),0,0);
/* temp1 = c*out->me[i][j] - s*out->me[k][j]; */
  
#pragma omp parallel for private (j) firstprivate (i,k,c)
...
ZMAT *mat,int i,int k,double c,complex s,ZMAT *out)
{
  ...
  complex y;
  if (mat == ((ZMAT *)((void *)0))) 
    ev_err("zgivens.c",8,167,"zrot_cols",0);
  if (i < 0 || i >= mat -> n || k < 0 || k >= mat -> n) 
    ev_err("zgivens.c",10,169,"zrot_cols",0);
  if (mat != out) 
    out = _zm_copy(mat,(zm_resize(out,(mat -> m),(mat -> n))),0,0);
