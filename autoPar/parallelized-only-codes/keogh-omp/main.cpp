...
// host side memory
  ...
  for (int i = 0; i <= N - 1; i += 1) {
    subject[i] = ((float )(rand())) / ((float )2147483647);
  }
  for (int i = 0; i <= N - M + 1 - 1; i += 1) {
    avgs[i] = ((float )(rand())) / ((float )2147483647);
  }
  for (int i = 0; i <= N - M + 1 - 1; i += 1) {
    stds[i] = ((float )(rand())) / ((float )2147483647);
  }
  for (int i = 0; i <= M - 1; i += 1) {
    upper_bound[i] = ((float )(rand())) / ((float )2147483647);
  }
  for (int i = 0; i <= M - 1; i += 1) {
    lower_bound[i] = ((float )(rand())) / ((float )2147483647);
  }
...
#pragma omp parallel for private (i_nom_6)
      for (int idx = 0; idx <= N - M + 1 - 1; idx += 1) {
// obtain statistics
        float residues = 0;
        float avg = avgs[idx];
        float std = stds[idx];
        
#pragma omp parallel for reduction (+:residues) firstprivate (avg,std)
        for (int i = 0; i <= M - 1; i += 1) {
