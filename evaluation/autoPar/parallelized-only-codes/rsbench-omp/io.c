...
// Calculate Estimate of Memory Usage
  ...
  if (input . simulation_method == 1) {
    printf("Particles:                   ");
    fancy_int(input . particles);
...
int validate_and_print_results(Input input,double runtime,unsigned long vhash,double kernel_time)
{
  ...
  if (input . simulation_method == 1) {
   ...
   else if (input . simulation_method == 2) {
    large = 358389;
    small = 880018;
  }
  if (input . HM == LARGE) {
