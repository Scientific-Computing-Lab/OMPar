...
{
  return {v1 . x + v2 . x + v3 . x + v4 . x + v5 . x + v6 . x, v1 . y + v2 . y + v3 . y + v4 . y + v5 . y + v6 . y, v1 . z + v2 . z + v3 . z + v4 . z + v5 . z + v6 . z, v1 . w + v2 . w + v3 . w + v4 . w + v5 . w + v6 . w};
}
        ...
        float4 shm_buffer[1536];
{
          ...
          g_data[globaltid] = reduce_vector(shm_buffer[tid + 0 * blk],shm_buffer[tid + 1 * blk],shm_buffer[tid + 2 * blk],shm_buffer[tid + 3 * blk],shm_buffer[tid + 4 * blk],shm_buffer[tid + 5 * blk]);
        }
      }
    }
    auto end = std::chrono::_V2::system_clock::now();
    time_shmem_128b = (std::chrono::duration_cast< std::chrono::nanoseconds  , int64_t  , std::nano  > ((end-start)) . count()) / ((double )n);
...
#pragma omp parallel for reduction (+:sum)
  for (long i = 0; i <= size - 1; i += 1) {
    sum += c[i];
  }
  if (sum != 21256458760384741137729978368.00) 
    printf("checksum failed\n");
  printf("Memory throughput\n");
  const long long operations_bytes = (((6LL + (4 * 5 * 1024) + 6) * size) * (sizeof(float )));
  const long long operations_128bit = (6LL + (4 * 5 * 1024) + 6) * size / 4;
