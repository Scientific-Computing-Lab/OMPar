...
uint calNumEigenValuesLessThan(const float *diagonal,const float *offDiagonal,const uint length,const float x)
{
  uint count = 0;
  float prev_diff = diagonal[0] - x;
  count += ((prev_diff < 0?1 : 0));
  
#pragma omp parallel for reduction (+:count) firstprivate (length,x)
  for (uint i = 1; i <= length - 1; i += 1) {
...
uint eigenValueCPUReference(float *diagonal,float *offDiagonal,uint length,float *eigenIntervals,float *newEigenIntervals,float tolerance)
{
  uint offset = 0;
  for (uint i = 0; i <= length - 1; i += 1) {
    uint lid = 2 * i;
    uint uid = lid + 1;
    uint eigenValuesLessLowerBound = calNumEigenValuesLessThan(diagonal,offDiagonal,length,eigenIntervals[lid]);
    uint eigenValuesLessUpperBound = calNumEigenValuesLessThan(diagonal,offDiagonal,length,eigenIntervals[uid]);
    uint numSubIntervals = eigenValuesLessUpperBound - eigenValuesLessLowerBound;
    if (numSubIntervals > 1) {
      float avgSubIntervalWidth = (eigenIntervals[uid] - eigenIntervals[lid]) / numSubIntervals;
      for (uint j = 0; j <= numSubIntervals - 1; j += 1) {
...
#pragma omp parallel for firstprivate (tolerance)
...
void computeGerschgorinInterval(float *lLimit,float *uLimit,const float *diagonal,const float *offDiagonal,const uint length)
{
  float lowerLimit = diagonal[0] - std::fabs(offDiagonal[0]);
  float upperLimit = diagonal[0] + std::fabs(offDiagonal[0]);
  for (uint i = 1; i <= length - ((unsigned int )1) - 1; i += 1) {
    float r = std::fabs(offDiagonal[i - 1]) + std::fabs(offDiagonal[i]);
    lowerLimit = (lowerLimit > diagonal[i] - r?diagonal[i] - r : lowerLimit);
    upperLimit = (upperLimit < diagonal[i] + r?diagonal[i] + r : upperLimit);
  }
  lowerLimit = (lowerLimit > diagonal[length - 1] - std::fabs(offDiagonal[length - 2])?diagonal[length - 1] - std::fabs(offDiagonal[length - 2]) : lowerLimit);
  upperLimit = (upperLimit < diagonal[length - 1] + std::fabs(offDiagonal[length - 2])?diagonal[length - 1] + std::fabs(offDiagonal[length - 2]) : upperLimit);
   *lLimit = lowerLimit;
   *uLimit = upperLimit;
}
